...
import { VenueType, VENUE_TYPE_LABELS, type Venue } from '@/types';
...
export default function EditVenue() {
  ...
  const { data: venue, isLoading } = useQuery<Venue>({
    queryKey: ['owner-venue', id],
    queryFn: async () => {
      const { data } = await api.get(`/owner/venues/${id}`);
      return data.data;
    },
  });
  ...
  useEffect(() => {
    if (venue) {
      reset({
        ...
        pricePerHour: venue.pricePerHour ?? undefined,
        priceHalfDay: venue.priceHalfDay ?? undefined,
        priceFullDay: venue.priceFullDay ?? undefined,
  ...
  const { mutate: updateVenue, isPending } = useMutation({
    mutationFn: (data: FormData) => api.put(`/owner/venues/${id}`, data),
    onSuccess: () => {
  ...
  if (!venue) return <div className="pt-16 min-h-screen flex items-center justify-center"><p className="text-muted">Venue not found.</p></div>;

  return (
    <div className="min-h-screen bg-slate-50 pt-16">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-10">
        <h1 className="font-display text-3xl font-semibold text-navy mb-2">Edit Venue</h1>
        <p className="text-muted text-sm mb-8">Changes will be visible immediately after saving.</p>

        <form onSubmit={handleSubmit((data) => updateVenue(data))} className="space-y-8">
          ...
          <div className="flex gap-3">
            <Button type="button" variant="outline" fullWidth onClick={() => navigate('/owner/venues')}>
              Cancel
            </Button>
